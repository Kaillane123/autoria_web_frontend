import tarefaView from "../view/tarefaIntercambista.js";
...
async function cadastrarTarefa(event) {
  ...
  const descricaoValor = document.getElementById("tarefa_cpf_formulario").value;
  const dataDenas = document.getElementById("tarefa_data_formulario").value;

  const novaTarefa = { nome: tituloValor, cpf: descricaoValor, data_nas:dataDenas };

  try {
    await fetch(`${API_BASE_URL}/intercambista`, {
      method: "POST",
...
/**
 * Renderiza a lista de intercambista.
...
async function renderizarListaTarefas(componentePrincipal) {
  try {
    const response = await fetch(API_BASE_URL + "/intercambista");
    const tarefasBD = await response.json();
    console.log(tarefasBD)

    const intercambista = tarefasBD.map((row) => {
      return {
        id: row.id,
        nome: row.nome,
        cpf: row.cpf,
    ...
    componentePrincipal.innerHTML = tarefaView.renderizarTabela(intercambista);
    $("#cadastrarIntercambista").addEventListener("click", ev => {
      ev.preventDefault()
    ...
    console.error("Erro ao buscar intercambista:", error);
  }
}
 ...
 * Após a exclusão bem-sucedida, a lista de intercambista é atualizada.
 * @param {string} id - ID da tarefa a ser excluída.
 */
async function excluirTarefa(id) {
  try {
    const response = await fetch(`${API_BASE_URL}/intercambista/${id}`, { method: "DELETE" });
...
async function buscarTarefa(id) {
  try {
    const response = await fetch(`${API_BASE_URL}/intercambista/${id}`);
    const tarefasBD = await response.json();
...
async function atualizarTarefa(event) {
  ...
  const tarefa = {id: idValor, nome: tituloValor,cpf: descricaoValor, data_nas:dataDenas};
